...
export function getWorkflowFlowData(req) {
    return api.get(config.getNodeTrackUrl(req.procInstId), {});
}
/**
 * 审批
 * @param taskId 任务id
 * @param req
 */
export function approveByTaskId(taskId, req) {
    return api.post(config.APPROVE_BY_TASKID_URL(taskId), req, false);
}
/**
 * 审批
 ...
 * @param req
 */
export function approveByBusinessKey(businessKey, req) {
    return api.post(config.APPROVE_BY_BUSSINESSKEY_URL(businessKey), req, false);
}
/**
 * 获取上一节点，或其它节点的信息
 * @param req
